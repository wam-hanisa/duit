...
- **⚠️ Auto-evolution overfitting risk (Darwin / evolveThresholds)**: `lessons.json` contains AUTO-EVOLVED lessons like *"maxVolatility=2.6 — losers clustered at ~2.2, tightened 2.8→2.6"*. Two problems: (1) **it's overfitting to noise** — bucket analysis of the 133 snapshot records shows the 2.5–3.0 vol band is one of the *best* (+0.42% avg, 0 catastrophes), so tightening to 2.6 would discard winners; the "losers at 2.2" was a small-sample point estimate, not a robust pattern. (2) **it never applied** — `evolveThresholds()` uses the wrong key name (no-op, see above), so the live `maxVolatility` is **4** while the lesson *claims* 2.6. `evolveThresholdsEnabled` defaults OFF (good) — keep it off and tune volatility manually from data. **Cross-window non-replication proof**: the "vol ≥ 3.5 is bad" signal that looked clean on the full 282-deploy set did NOT hold on the recent 133-record window (there 2.0–2.5 was the worst band, 3.5+ only mildly negative). This is exactly the overfitting trap [@dikibagast](https://x.com/dikibagast)'s thread warns about — a signal that fits old data but fails on new data. Do not hard-code a `maxVolatility` cut from a single window; require the pattern to replicate across windows first.
